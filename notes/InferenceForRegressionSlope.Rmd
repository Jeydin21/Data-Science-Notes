---
title: "Inference for Regression Slope Notes"
output:
  html_document:
    toc: true
    theme: darkly
...
```{r}
porsche.lm <- lm(Price ~ Mileage, data = porsche)
```
...
```{r}
confint(porsche.lm, 1)
confint(porsche.lm, 2)
```
...
Consider a Porsche with 50,000 miles. What is the 95% prediction interval for the price of this car?
...
predict(porsche.lm, newData, interval = "confidence", level = .95)
```

### Next, find the 95% Prediction Interval
```{r}
predict(porsche.lm, newData, interval = "prediction", level = .95)
```
